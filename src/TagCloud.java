...
/**
 * A visual representation for the number of times various words occur in a
 * given document.
 * 
 *  In a tag cloud, the most common English words are usually ignored (e.g. 
 *  the, a, of), since they do not give any useful information about the topic
 * of the document being visualized.
 * 
...
public class TagCloud {
    ...
    public void topWords (int numWordsToKeep, int groupSize) {
        ...
        for (int k = 0; k < myTagWords.size(); k++) {
        	...
        	myTagWords.set(k, new SimpleEntry<String, Integer>(word.getKey(), word.getValue() / groupSize));
        }
        // sort alphabetically
    	myTagWords.sort(new Comparator<Entry<String, Integer>>() {
            @Override
    ...
    private String sanitize (String word) {
        return word.replaceFirst("^" + PUNCTUATION, "").replaceFirst(PUNCTUATION + "$", "").toLowerCase();
    }
    ...
    public static void main (String[] args) {
        ...
        try {
        	TagCloud cloud = new TagCloud(new Scanner(TagCloud.class.getResourceAsStream(DEFAULT_IGNORE_FILE)));
